 ...
 * {@link VocableData} is bound to exactly one {@link LanguagePlugin}. It contains all data for one
...
public class VocableData
{
  ...
  private ManagedData<String> mWordTypeIdentifier;
  ...
  private VocableData () 
  {
    mWordTypeIdentifier = new ManagedData<String> ();
  }
  ...
  public AbstractUserInput getUserInput ()
  {
    ...
    if (mUserInput == null)
    {
      ...
      AbstractWordType wordType = mForLanguage.getLanguagePlugin ().getValue ().getWordTypeFor (mWordTypeIdentifier.getValue ());
  ...
  public AbstractWordType getWordType ()
  {
    if (mWordType == null)
    {
      if (mWordTypeIdentifier.canRead ())
      {
        mWordType = getLanguage ().getLanguagePlugin ().getValue ()
          .getWordTypeFor (mWordTypeIdentifier.getValue ());
        if (mWordType == null)
        {
  ...
  public void setWordTypeIdentifier (String wordTypeIdentifier)
  {
    ...
    mWordTypeIdentifier.setValue (wordTypeIdentifier);
  }
  
  @SuppressWarnings("unused") private long getId ()        {return mId;}
  @SuppressWarnings("unused") private void setId (long id) {mId = id;}
}
