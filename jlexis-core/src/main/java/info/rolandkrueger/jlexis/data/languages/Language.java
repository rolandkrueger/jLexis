 ...
 * creating a new {@link LearningUnit} the user typically selects one or more foreign languages she
 ...
 * the user selects a default language plugin for her new {@link LearningUnit}, she is asked to
 * provide the name of the language she wants to handle with this plugin. With this more languages
 * can be learned than language plugins are available. Each {@link LearningUnit} can manage an own set
 * of {@link Language} objects which are independent of other {@link LearningUnit}s. 
 * </p>
 * 
...
public class Language
{
   ...
   */
  private ManagedData<String> mPluginIdentifier;
  /**
   ...
   */
  private ManagedData<Integer> mPluginVersion;
  ...
  protected Language ()
  {
    mPlugin           = new ManagedData<LanguagePlugin> ();
    mLanguageName     = new ManagedData<String>         ();
    mPluginIdentifier = new ManagedData<String>         ();
    mPluginVersion    = new ManagedData<Integer>        ();
  }
   ...
   * 
   * @param pluginForThisLanguage
   *          the plugin that corresponds to this {@link Language}. 
   */
  ...
  public void setLanguageName (String languageName)
  {
    ...
    mLanguageName.setValue (languageName);
  }

  public void setLanguagePlugin (LanguagePlugin plugin)
  {
    mPlugin.setValue    (plugin);
    setPluginIdentifier (plugin.getIdentifier ());
    setPluginVersion    (plugin.getVersionNumber ());
  }

  public ManagedData<LanguagePlugin> getLanguagePlugin ()
  {
    if (mPlugin.isUndefined ())
    {
  ...
  {
    return mPluginIdentifier.getValue ();
  }

  public void setPluginIdentifier (String pluginIdentifier)
  {
    mPluginIdentifier.setValue (pluginIdentifier);
  }

  public Integer getPluginVersion ()
  {
    return mPluginVersion.getValue ();
  }

  public void setPluginVersion (int pluginVersion)
  {
    mPluginVersion.setValue (pluginVersion);
  }
  
  @Override
  public String toString ()
  {
    if (mLanguageName.isUndefined ()) return "?";
  ...
  private long getId () {return mID;}
  @SuppressWarnings("unused")
  private void setId (long id) {mID = id;}
}
